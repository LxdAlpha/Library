...
%>
  ...
  <tr>
    <td>
      <a href="#" onClick="window.open('${path}/manager/manager_add.html','','width=292,height=175')">添加管理员信息</a> </td>
  </tr>
</table>
 <%
}else{
...
<%
  while(it.hasNext()){
    ...
    Manager managerForm=(Manager)it.next();
  ...
  <tr>
    ...
    <td align="center"><%if(!name.equals("admin")){ %><a href="#" onClick="window.open('${path}/manager/managerModifyQuery.html?id=<%=ID%>','','width=292,height=175')">权限设置</a><%}else{%>&nbsp;<%}%></td>
    <td align="center">
	<%if(!name.equals("admin")){ %><a href="${path}/manager/managerDel.html?id=<%=ID%>">删除</a><%}else{%>&nbsp;<%}%></td>
  </tr>
<%
  }
}
